...
async function handleGetDiary(req, res) {
  ...
  const result = await query('SELECT de.id, de.entry_text, de.created_at, u.name FROM diary_entries de JOIN users u ON de.user_id = u.id WHERE de.user_id = $1 ORDER BY de.created_at DESC LIMIT $2', [user_id, parseInt(limit)]);
...
async function handleAddDiary(req, res) {
  ...
  await query('INSERT INTO diary_entries (user_id, entry_text, created_at) VALUES ($1, $2, NOW())', [user_id, entry_text]);
...
async function handleDeleteDiary(req, res) {
  ...
  const result = await query('DELETE FROM diary_entries WHERE id = $1 AND user_id = $2', [entry_id, user_id]);
...
async function handleGetMedicines(req, res) {
  ...
  if (!user_id) return res.status(400).json({ success: false, message: 'User ID required' });
  const result = await query('SELECT um.id, um.quantity, m.medicine_name FROM user_medicines um JOIN medicines m ON um.medicine_id = m.id WHERE um.user_id = $1 AND um.quantity > 0 ORDER BY m.medicine_name', [user_id]);
...
async function handleTakeMedicine(req, res) {
  ...
  try {
    ...
    const check = await client.query('SELECT um.quantity, m.medicine_name, um.medicine_id FROM user_medicines um JOIN medicines m ON um.medicine_id = m.id WHERE um.id = $1 AND um.user_id = $2', [user_medicine_id, user_id]);
    if (check.rowCount === 0 || check.rows[0].quantity <= 0) throw new Error('Invalid medicine or no quantity');
    const med = check.rows[0];
    await client.query('UPDATE user_medicines SET quantity = quantity - 1 WHERE id = $1', [user_medicine_id]);
    await client.query('INSERT INTO medicine_logs (user_id, medicine_id, taken_at) VALUES ($1, $2, NOW())', [user_id, med.medicine_id]);
    await client.query('INSERT INTO diary_entries (user_id, entry_text, created_at) VALUES ($1, $2, NOW())', [user_id, `Took ${med.medicine_name}`]);
    ...
    // Note: The client is from the pool, ending it is okay or we could release it.
    // In our db_helper.js, getClient() ends the client at the end of query(), 
    // but here we used getClient() manually. We should end it.
    await client.end();
  }
}
...
async function handleGetLogs(req, res) {
  ...
  if (!user_id) return res.status(400).json({ success: false, message: 'User ID required' });
  const result = await query('SELECT ml.taken_at, m.medicine_name FROM medicine_logs ml JOIN medicines m ON ml.medicine_id = m.id WHERE ml.user_id = $1 ORDER BY ml.taken_at DESC LIMIT $2', [user_id, parseInt(limit)]);
